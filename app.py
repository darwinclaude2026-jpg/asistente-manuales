...
def generar_embeddings(textos, task_type):
    """Pide embeddings a la API de Gemini en lotes, con reintentos si hay límite de cuota."""
    vectores = []
    for i in range(0, len(textos), EMBED_BATCH):
        ...
        while True:
            try:
                ...
                intentos += 1
                if "RESOURCE_EXHAUSTED" in str(e) and intentos <= 5:
                    time.sleep(15)
                    continue
                raise
        vectores.extend([e.values for e in resultado.embeddings])
        time.sleep(1)  # pequeña pausa entre lotes para no saturar la cuota
