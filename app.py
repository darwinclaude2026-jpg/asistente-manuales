...
def build_index():
    ...
    if cached_chunks is not None:
        return client, cached_chunks, cached_vectors, False  # False = ya estaba guardado, no hubo que gastar cuota
    ...
    return client, chunks, vectors, True  # True = recién calculado ahora
...
client, chunks, vectors, recien_calculado = build_index()
...
if recien_calculado and os.path.exists(INDEX_FILE):
    with open(INDEX_FILE, "rb") as f:
        st.info(
            "✅ Índice generado. Para que no se vuelva a gastar cuota de la API en cada "
            "reinicio de la app, descarga este archivo y súbelo a la raíz de tu repositorio "
            "de GitHub (junto a app.py). La próxima vez la app lo cargará instantáneamente."
        )
        st.download_button(
