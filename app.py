...
def build_index():
    ...
    if not chunks:
        return client, [], None, "sin_manuales", 0
    ...
    if resume_from >= len(chunks):
        return client, chunks, existing_vectors, "completo_desde_cache", len(chunks)
    ...
    total_esperado = len(chunks)

    if len(total_vectores) == 0:
        return client, [], None, "limite_alcanzado_sin_avance", total_esperado
    ...
    if not completo:
        return client, chunks_procesados, total_vectores_norm, "limite_alcanzado_parcial", total_esperado
    return client, chunks, total_vectores_norm, "completo_recien_calculado", total_esperado
...
# Contar fragmentos es una operación local (no usa la API, no gasta cuota),
# así que lo mostramos de inmediato para que sepas el total sin esperar nada.
_chunks_preview = extract_chunks_from_pdfs(MANUALS_DIR)
if _chunks_preview:
    st.caption(f"📄 Se detectaron **{len(_chunks_preview)} fragmentos** en tus manuales (esto no gasta cuota de la API).")

client, chunks, vectors, estado, total_esperado = build_index()
...
if estado in ("completo_recien_calculado", "limite_alcanzado_parcial") and os.path.exists(INDEX_FILE):
    ...
    if estado == "completo_recien_calculado":
        st.info(
            f"✅ Índice generado por completo ({total_esperado} de {total_esperado} fragmentos). "
            "Para que no se vuelva a gastar cuota de la API en cada reinicio, descarga este archivo "
            "y súbelo a la raíz de tu repositorio de GitHub (junto a app.py)."
        )
    else:
        faltan = total_esperado - len(chunks)
        st.warning(
            f"⚠️ Se alcanzó el límite gratuito de la API a mitad de camino. "
            f"**Progreso: {len(chunks)} de {total_esperado} fragmentos** (quedan {faltan} pendientes). "
            f"Descarga este archivo parcial y súbelo a GitHub como `manuales_index.npz` — la próxima vez "
            f"(cuando se libere la cuota) la app continuará automáticamente desde donde quedó."
        )
    st.download_button(
